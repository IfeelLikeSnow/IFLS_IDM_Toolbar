...
--     3) Writes/updates library + require shim:
--         Scripts/IFLS_IDM_Toolbar/lib/IFLS_FXParamIndex.lua
--         Scripts/IFLS_FXParamIndex.lua  (shim for require)
--
--   Output data:
--     <REAPER_RESOURCE_PATH>/Scripts/IFLS_IDM_Toolbar/_data/fx_param_index.tsv
--     <REAPER_RESOURCE_PATH>/Scripts/IFLS_IDM_Toolbar/_data/fx_param_index.json
--
--   WARNING:
...
local function ensure_dir(path) r.RecursiveCreateDirectory(path, 0); return path end
...
for i = 1, #installed do
  ...
  local fx_entry = { index = i - 1, name = fx.name, ident = fx.ident, params = {}, param_count = 0 }
  ...
  if ok_add and type(fx_idx) == "number" and fx_idx >= 0 then
    ...
    for p = 0, num - 1 do
      ...
      fx_entry.params[#fx_entry.params+1] = {
        index = p, name = pname or "", ident = pident,
        val = val, min = minv, max = maxv, formatted = formatted or "",
      }

      local function clean_tab(s) return (tostring(s or ""):gsub("\t", " ")) end
      f_tsv:write(("%d\t%s\t%s\t%d\t%s\t%s\t%.10f\t%.10f\t%.10f\t%s\n"):format(
        i - 1, clean_tab(fx.name), clean_tab(fx.ident),
        p, clean_tab(pname), clean_tab(pident),
        val, minv, maxv, clean_tab(formatted)
      ))
    end
...
-- Step 3: write JSON
----------------------------------------------------------------
if not file_write(json_path, json_encode(data)) then
  r.ShowMessageBox("Couldn't write JSON:\n" .. json_path, "IFLS FX Param Index", 0)
end

----------------------------------------------------------------
-- Step 4: ensure lib + shim exist (write/overwrite to stay in sync)
----------------------------------------------------------------
local lib_src = ([[-- IFLS_FXParamIndex.lua
...
local shim_src = ([[-- IFLS_FXParamIndex.lua (shim)
